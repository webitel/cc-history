...
const { hasReadAccess: hasAuditFormReadAccess } = useUserAccessControl(
	WtObject.AuditForm,
);
...
const showEvaluationForm = computed(() => {
	...
	if (!scorecard.value.questions) {
		...
		alreadyRated &&
		(!isEditingEvaluationResult.value || !hasRatingUpdateAccess.value)
	) {
		return false;
	}
...
const updateEvaluationResult = async (evaluationResult: EngineAuditRate) => {
	await store.dispatch(
		`${props.namespace}/UPDATE_EVALUATION`,
		evaluationResult,
	);
};
...
const deleteEvaluationResult = async () => {
	await store.dispatch(
		`${props.namespace}/DELETE_EVALUATION`,
		props.call.rateId,
	);
	clearScorecard();
};
