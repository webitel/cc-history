...
const sortDatasets = (datasets) =>
  new Map(
    [...datasets.entries()].sort(
      (dataset1, dataset2) => dataset2[1].sum - dataset1[1].sum,
    ),
  );
...
const normalizeDashboardData = ({
  ...
  const dataSources = data.reduce(
    (wrapper, item) => {
      ...
      if (datasets.data.length < wrapper.dates.size) {
        datasets.data = datasets.data.concat(
          new Array(wrapper.dates.size - datasets.data.length - 1).fill(
            defaultValue,
          ),
        );
      }
  ...
  dataSources.datasets.forEach((dataset) => {
    if (dataset.data.length < dataSources.dates.size) {
      dataset.data = dataset.data.concat(
        new Array(dataSources.dates.size - dataset.data.length).fill(
          defaultValue,
        ),
      );
    }
