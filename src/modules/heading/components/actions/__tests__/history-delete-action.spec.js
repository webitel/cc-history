...
describe('HistoryDeleteAction', () => {
  ...
  it('renders a component', () => {
    const wrapper = shallowMount(HistoryDeleteAction, { props });
    expect(wrapper.isVisible()).toBe(true);
  });
  it('delete action is enabled is there`s some selected', () => {
    selected = [{ files: {} }];
    const wrapper = shallowMount(HistoryDeleteAction, { props: { selected } });
    const btn = wrapper.findComponent({ name: 'wt-button-select' });
